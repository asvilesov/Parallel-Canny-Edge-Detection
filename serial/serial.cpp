...
//create 2D array
	...
	float **grad_img = new float*[height];
	float **nonmaxed_img = new float*[height];
	float **canny_img = new float*[height];

    for(int i = 0; i < height; ++i){
    	img[i] = new float[width];
    	blur_img[i] = new float[width];
		theta[i] = new float[width];
		grad_img[i] = new float[width];
		nonmaxed_img[i] = new float[width];
		canny_img[i] = new float[width];
    }
...
//check for atan bug
	sobel_filter(blur_img, grad_img, theta, height, width);
	visualize(grad_img, height, width);

	non_max_suppression(grad_img, nonmaxed_img, theta, height, width);
	visualize(nonmaxed_img, height, width);

	double_threshold(nonmaxed_img, canny_img, theta, height, width);
	hysteresis(canny_img, height, width);

	visualize(canny_img, height, width);
